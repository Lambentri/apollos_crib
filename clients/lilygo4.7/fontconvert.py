...
parser.add_argument("--interval", dest="interval", action="store", help="a json array of intervals to convert")
...
if args.interval:
    try:
        with open(args.interval, "r") as f:
            intervals = f.read()
            parsed = [[int(j, 16) for j in i] for i in json.loads(intervals)]
            doubled = [[i[0],i[0]] if len(i) == 1 else i for i in parsed ]
            intervals = doubled
    except Exception as e:
        print(f"failed to open interval file {e}", file=sys.stderr)
        intervals = [
            (32, 126),
            (160, 255),
            # (0x2500, 0x259F),
